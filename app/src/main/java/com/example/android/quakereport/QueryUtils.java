    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            for(int i= 0; i < quakes.length(); i++){
               Log.i("QueryUtils", quakes.getJSONObject(i).toString());
               JSONObject thisQuake = quakes.getJSONObject(i);
               JSONObject properties  = thisQuake.getJSONObject("properties");
               double magnitude = properties.getDouble("mag");
               String location = properties.getString("place");
               long time = properties.getLong("time");
               String url = properties.getString("url");
               eq = new Earthquake(magnitude, location, time, url);

                Log.i("Quak", eq.toString() );
                earthquakes.add(eq);
            }
